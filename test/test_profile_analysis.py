...
def test_types():
    ...
    assert (
        isinstance(profuq["mean"], UQResult)
        and profuq["scale"] is None
        and isinstance(profuq, dict)
    )
...
def test_globalmodel():
    ...
    model = link(model, mean=["mean_1", "mean_2"], std=["std_1", "std_2"])
...
def test_grids():
    ...
    profuq = profile_analysis(
        model, y, r, parameters="mean", grids=grid, noiselvl=sigma
    )
