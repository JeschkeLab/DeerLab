...
def test_threemodels_default_linear():
    ...
    assert [
        scale_par in model._parameter_list()
        for scale_par in ["scale_1", "scale_2", "scale_3"]
    ]
...
def test_threemodels_addweights_values():
    ...
    assert all(
        [
            np.allclose(response, ref)
            for response, ref in zip(
                [response1, response2, response3], [ref1, ref2, ref3]
            )
        ]
    )
...
def test_vec_addweights():
    ...
    response = model(r_1=x, r_2=x, Pvec_1=ref1, Pvec_2=ref2, weight_1=1, weight_2=1)
