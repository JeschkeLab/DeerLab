...
for n, sample in enumerate(samples):
    ci95[n, :] = np.array(
        [np.percentile(samples[n], 2.5), np.percentile(samples[n], 97.5)]
    )
    ci90[n, :] = np.array(
        [np.percentile(samples[n], 5.0), np.percentile(samples[n], 95.0)]
    )
    ci50[n, :] = np.array(
        [np.percentile(samples[n], 25), np.percentile(samples[n], 75)]
    )
...
threshold = lambda coverage: σ**2 * chi2.ppf(coverage, df=1) + likelihood2obj(
    max(pdf1)
)
