...
def test_mixed_vector_call_keywords():
    ...
    response = model(
        scale=5,
        gaussian=gauss2(mean1=3, mean2=4, std1=0.2, std2=0.3, amp1=0.5, amp2=0.4),
    )
...
def gauss2_design_axis(axis, mean1, mean2, std1, std2):
    return np.atleast_2d(
        [gauss_axis(axis, mean1, std1), gauss_axis(axis, mean2, std2)]
    ).T
...
mock_data_fcn = lambda axis: gauss2_axis(
    axis, mean1=3, mean2=4, std1=0.5, std2=0.2, amp1=0.5, amp2=0.6
)
...
def _getmodel_axis(type, vec=50):
    if type == "parametric":
        ...
        model = Model(lambda x: np.eye(len(x)), constants="x")
        model.addlinear(
            "dist", lb=0, vec=vec, normalization=lambda dist: dist / np.sum(dist)
        )
    return model
...
def test_fit_evaluate_callable():
    ...
    fcn = (
        lambda mean1, mean2, std1, std2, amps: gauss2_design_axis(
            x, mean1, mean2, std1, std2
        )
        @ amps
    )
...
def test_pickle_fitresult():
    ...
    try:
        ...
        assert np.allclose(pickled_fitResult.model.real, y.real) and np.allclose(
            pickled_fitResult.model.imag, y.imag
        )
