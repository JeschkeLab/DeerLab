...
def test_preserve_original():
    ...
    assert model1._parameter_list() == ["mean", "std"] and model2._parameter_list() == [
        "conc",
        "lam",
    ]
...
def test_twomodels_param_names():
    ...
    assert all(
        [
            ...
            for str in ["mean_1", "mean_2", "std_1", "std_2"]
        ]
    )
...
def test_threemodels_param_names():
    ...
    assert all(
        [
            str in model._parameter_list()
            for str in ["mean_1", "mean_2", "mean_3", "std_1", "std_2", "std_3"]
        ]
    )
...
def test_threemodels_call():
    ...
    assert all(
        [np.allclose(response[n], ref) for n, ref in enumerate([ref1, ref2, ref3])]
    )
...
model_vec_normalized.addlinear(
    "Pvec", vec=100, lb=0, normalization=lambda Pvec: Pvec / np.sum(Pvec)
)
...
def test_vec_param_names():
    ...
    model = merge(model1, model2)
    assert all(
        [str in model._parameter_list() for str in ["mean_1", "std_1", "Pvec_2"]]
    )
...
def test_twomodels_normalization_values():
    ...
    assert np.isclose(results.Pvec_1_scale, scale1) and np.isclose(
        results.Pvec_2_scale, scale2
    )
...
def test_twomodels_vec_equal_global():
    ...
    assert np.allclose(results.model, globalresults.model) and np.allclose(
        results.Pvec, globalresults.Pvec_1, globalresults.Pvec_2
    )
...
def test_twomodels_equal_global():
    ...
    assert np.allclose(result.model, truth, globalresult.model) and np.allclose(
        result.mean, globalresult.mean_1
    )
