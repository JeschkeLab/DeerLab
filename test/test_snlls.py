...
def assert_multigauss_SNLLS_problem(nonlinearconstr=True, linearconstr=True):
    ...
    # Separable LSQ fit
    fit = snlls(
        V, lambda p: Kmodel(p, t, r), nlpar0, lb, ub, lbl, ubl, reg=False, uq=False
    )
...
def test_regularized():
    ...
    # Separable LSQ fit
    fit = snlls(
        V, lambda lam: dipolarkernel(t, r, mod=lam), nlpar0, lb, ub, lbl, uq=False
    )
    Pfit = fit.lin
...
def assert_confidence_intervals(pci50, pci95, pfit, lb, ub):
    ...
    if not np.all(p95lb <= pfit) and not np.all(p50lb <= pfit):
        errors.append(
            "Some fitted values are below the lower bound of the confidence intervals."
        )
    if not np.all(p95ub >= pfit) and not np.all(p50ub >= pfit):
        errors.append(
            "Some fitted values are over the upper bound of the confidence intervals."
        )
...
def test_regularized_global():
    ...
    # Separable LSQ fit
    fit = snlls(
        [V1, V2], globalKmodel, par0, lb, ub, lbl, ubl, uq=False, weights=[1, 1]
    )
    Pfit = fit.lin
...
def test_size_control():
    ...
    with pytest.raises(RuntimeError):
        fit = snlls(
            V,
            lambda lam: dipolarkernel(twrong, r, mod=lam),
            nlpar0,
            lb,
            ub,
            lbl,
...
def test_cost_value():
    ...
    assert (
        isinstance(fit.cost, float)
        and np.round(fit.cost / np.sum(fit.residuals**2), 5) == 1
    )
...
def test_confinter_values():
    ...
    ci_match = (
        lambda ci, ci_ref, truth: np.max(abs(np.array(ci) - np.array(ci_ref))) / truth
        < 0.01
    )
...
def test_extrapenalty():
    ...
    beta = 0.05
    compactness_penalty = lambda _, plin: beta * np.sqrt(
        plin * (r - np.trapz(plin * r, r)) ** 2 * dr
    )
...
def test_multiple_penalties():
    ...
    beta = 0.05
    R = 0.5
    compactness_penalty = lambda pnonlin, plin: beta * np.sqrt(
        plin * (r - np.trapz(plin * r, r)) ** 2 * dr
    )
    radial_penalty = (
        lambda pnonlin, plin: 1
        / R**2
        * (np.linalg.norm((pnonlin - param) / param - R)) ** 2
    )
...
def test_frozen_param():
    ...
    def Amodel(p):
        ...
        return np.atleast_2d(
            [dd_gauss.nonlinmodel(r, mean1, std1), dd_gauss.nonlinmodel(r, mean2, std2)]
        ).T
    ...
    fit_frozen = snlls(
        ...
        ub=[10, 10, 5, 5],
        lbl=[0, 0],
        nonlin_frozen=nonlin_frozen,
        lin_frozen=lin_frozen,
    )
...
def test_frozen_Nparam():
    ...
    assert (
        len(fit.nonlin) == 4
        and len(fit.lin) == 2
        and len(fit_frozen.nonlin) == 4
        and len(fit_frozen.lin) == 2
    )
...
def test_complex_model_complex_data():
    ...
    fitResult = snlls(
        y, model, par0=[2 * np.pi / 5, 4, 0.2], lb=[-np.pi, 1, 0.05], ub=[np.pi, 6, 5]
    )
...
def test_complex_model_uncertainty():
    ...
    ciwidth = np.sum(
        fitResult.modelUncert.ci(95)[:, 1] - fitResult.modelUncert.ci(95)[:, 0]
    )
