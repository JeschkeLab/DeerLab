...
from deerlab.dipolarmodel import (
    ExperimentInfo,
    dipolarpenalty,
    dipolarmodel,
    ex_4pdeer,
...
V3path = (
    1e5
    * dipolarkernel(t, r, pathways=[[0.5], [0.3, 0], [0.1, 2], [0.1, 5]], bg=Bfcn)
    @ Pr
)
...
V2harm = (
    1e5 * dipolarkernel(t, r, pathways=[[0.6], [0.3, 0, 1], [0.1, 2, 2]], bg=Bfcn) @ Pr
)
V3harm = (
    1e5
    * dipolarkernel(
        t, r, pathways=[[0.5], [0.3, 0, 1], [0.1, 2, 2], [0.1, 5, 3]], bg=Bfcn
    )
    @ Pr
)
...
V3pdeer = (
    1e5 * dipolarkernel(tdeer, r, pathways=[[0.6], [0.3, 0], [0.1, tau1]], bg=Bfcn) @ Pr
)
V4pdeer = (
    1e5
    * dipolarkernel(
        tdeer, r, pathways=[[0.6], [0.3, tau1], [0.1, tau1 + tau2]], bg=Bfcn
    )
    @ Pr
)
Vfwd5pdeer = (
    1e5
    * dipolarkernel(tdeer, r, pathways=[[0.6], [0.3, tau3], [0.1, tau1]], bg=Bfcn)
    @ Pr
)
Vrev5pdeer = (
    1e5
    * dipolarkernel(tdeer, r, pathways=[[0.6], [0.3, tau3], [0.1, tau2]], bg=Bfcn)
    @ Pr
)
Vsifter = (
    1e5
...
Vridme = (
    1e5
    * dipolarkernel(
        tridme, r, pathways=[[0.3], [0.5, 0], [0.1, tau2], [0.1, -tau1]], bg=Bfcn
    )
    @ Pr
)
...
def test_ex_3pdeer_fit():
    ...
    assert (
        np.allclose(V3pdeer, result.model, atol=1e-1)
        and ovl(result.P / 1e5, Pr) > 0.975
    )
...
def test_ex_4pdeer_fit():
    ...
    assert (
        np.allclose(V4pdeer, result.model, atol=1e-2)
        and ovl(result.P / 1e5, Pr) > 0.975
    )
...
def test_ex_rev5pdeer_fit():
    ...
    assert (
        np.allclose(Vrev5pdeer, result.model, atol=1e-2)
        and ovl(result.P / 1e5, Pr) > 0.975
    )
...
def test_ex_fwd5pdeer_fit():
    ...
    assert (
        np.allclose(Vfwd5pdeer, result.model, atol=1e-2)
        and ovl(result.P / 1e5, Pr) > 0.975
    )
...
def test_ex_sifter_fit():
    ...
    assert (
        np.allclose(Vsifter, result.model, atol=1e-2)
        and ovl(result.P / 1e5, Pr) > 0.975
    )
...
def test_ex_ridme_fit():
    ...
    assert (
        np.allclose(Vridme, result.model, atol=1e-2) and ovl(result.P / 1e5, Pr) > 0.975
    )
...
def test_orisel():
    ...
    Vmodel = dipolarmodel(t, r, dd_gauss, bg_hom3d, npathways=1)
    Vmodelorisel = dipolarmodel(
        t, r, dd_gauss, bg_hom3d, npathways=1, orisel=lambda theta: np.ones_like(theta)
    )
