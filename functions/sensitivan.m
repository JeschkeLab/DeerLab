...
for i = 1:nCombinations
    ...
    if isempty(nout)
        nout = getnargout(fcnHandle,argin);        
        evals = cell(1,nout);
    end
    ...
    for iOut = 1:nout
        ...
        try
            evals{iOut} = cat(1,evals{iOut},out);
        catch
...
percentile_ = @(v,p) interp1(linspace(0.5/length(v), 1-0.5/length(v), length(v))', sort(v), p*0.01, 'spline');
...
function nout = getnargout(fcnHandle,argin)
nout = nargout(fcnHandle);
if nout==-1 % for anonymous functions and those with fun(varargin)
  ...
  while true
    try
      ...
      break % done if call doesn't error
    catch
      nout = nout + 1;
    end
  end
elseif nout<-1 % fun(...,varargin)
  nout = abs(nout);
end
end
