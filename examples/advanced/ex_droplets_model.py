...
def dropletmodel(t):
    ...
    Vdis_model = dl.dipolarmodel(
        t, r, Pmodel=dl.dd_gauss, Bmodel=dl.bg_exp, npathways=2
    )
    # Liquid-droplet-state component model
    Vld_model = dl.dipolarmodel(
        t, r, Pmodel=dl.dd_gauss, Bmodel=dl.bg_strexp, npathways=2
    )
...
plt.fill_between(
    ts[0], fit.modelUncert[0].ci(95)[:, 0], fit.modelUncert[0].ci(95)[:, 1], alpha=0.3
)
plt.ylim([0.2, 1])
...
Vdis_fit = Vdismodel1(
    ...
    mean=fit.rmean_dis,
    std=fit.std_dis,
    reftime1=fit.reftime1,
    reftime2=fit.reftime2_1,
    lam1=fit.lam1,
    lam2=fit.lam2,
    scale=1,
)
...
plt.fill_between(
    ts[1], fit.modelUncert[1].ci(95)[:, 0], fit.modelUncert[1].ci(95)[:, 1], alpha=0.3
)
plt.ylim([0.2, 1])
plt.legend(frameon=False, loc="best")
plt.xlabel("Time t (μs)")
...
plt.plot(
    r,
    Pdis_fcn(fit.rmean_dis, fit.std_dis),
    label=f"Dispersed fraction {fit.eta*100:.1f}%",
    color=violet,
)
plt.fill_between(
    r, Pdis_uq.ci(95)[:, 0], Pdis_uq.ci(95)[:, 1], alpha=0.3, linewidth=0, color=violet
)
plt.plot(
    r,
    Pld_fcn(fit.rmean_ld, fit.std_ld),
    label=f"Liquid-droplet fraction {(1-fit.eta)*100:.1f}%",
    color=orange,
)
plt.fill_between(
    r, Pld_uq.ci(95)[:, 0], Pld_uq.ci(95)[:, 1], alpha=0.3, linewidth=0, color=orange
)
