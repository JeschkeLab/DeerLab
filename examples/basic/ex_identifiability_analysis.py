...
profile_long = dl.profile_analysis(
    Vmodel, Vexp, samples=10, parameters=["conc", "mod"], grids=grids
)
profile_short = dl.profile_analysis(
    Vmodel_truncated,
    Vexp_truncated,
    samples=10,
    parameters=["conc", "mod"],
...
for n, param in enumerate(["conc", "mod"]):
    ...
    plt.legend(
        ["Profile (long trace)", "Profile (short trace)", "Threshold"],
        frameon=False,
        loc="best",
    )
