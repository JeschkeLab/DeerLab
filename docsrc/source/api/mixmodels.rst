...
Combines the parametric model function handles ``model1``, ``model2``, ...,  ``modelN`` into a new parametric model function handle ``newmodel``. The models must be passed as a cell array of function handles and the parametric models must be of the type as the models distributed in DeerAnalysis. The returned function handle can be used for parametric model fitting.
