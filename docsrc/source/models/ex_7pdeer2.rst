...
.. autofunction:: deerlab.ex_models.ex_7pdeer2

Model
=========================================

This experiment model has three modulated pathways and an unmodulated contribution. The kernel is 

.. math::

    K(t,r) =
    [\Lambda_0 + \sum^2_{p=1} \lambda_p K_0(t-T_0^{(p)},r)]
    \prod^2_{p=1} B(t - T_0^{(p)},\lambda_p)

where :math:`T_0^{(1)}=0\;\mu s`, and :math:`T_0^{(2)}` are the refocusing times of the three modulated dipolar pathways at positive evolution times.
