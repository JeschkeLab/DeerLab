...
class UQResult:
    ...
    def __init__(
        ...
        if uqtype == "covariance":
            ...
            self.type = uqtype
            self.mean, self.median, self.std, self.covmat, self.nparam = (
                [] for _ in range(5)
            )
            return
        else:
            raise NameError(
                "uqtype not found. Must be: "
                "covariance"
                ", "
                "bootstrap"
                " or "
        ...
        # Profile-based CI specific fields
            ...
            std = [
                np.sqrt(np.trapz(pardist * (x - mean) ** 2, x))
                for x, pardist, mean in zip(xs, pardists, means)
            ]
            self.mean = means
        ...
        # Bootstrap-based CI specific fields
            ...
            covmat = (
                np.squeeze(samples).T @ np.squeeze(samples) / np.shape(samples)[0]
                - means * means.T
            )
            self.mean = means
            self.median = self.percentile(50)
            self.median = np.array(
                [
                    nth_samples[0]
                    if np.all(nth_samples == nth_samples[0])
                    else self.median[n]
                    for n, nth_samples in enumerate(samples.T)
                ]
            )
    ...
    def __getattribute__(self, attr):
        try:
            ...
            if (attr != "type" and not "__" in attr) and super(
                UQResult, self
            ).__getattribute__("type") == "void":
                # Check if the uncertainty quantification has been done, if not report that there is nothing in the object
                raise ValueError(
                    f"The requested attribute/method ('{attr}') is not available. Uncertainty quantification has not been calculated during the fit."
                )
    ...
    def pardist(self, n=0):
        ...
        if self.type == "bootstrap":
            ...
            if np.all(samplen == samplen[0]):
                ...
                # Generate kernel
                delta = np.maximum(
                    np.finfo(float).eps, (edges.max() - edges.min()) / (len(edges) - 1)
                )
        ...
        if self.type == "profile":
            ...
            obj2likelihood = (
                lambda f: 1
                / np.sqrt(σ * 2 * np.pi)
                * np.exp(-1 / 2 * (f - np.min(f)) / σ**2)
            )
            profileinterp = interp1d(
                profile["x"],
                profile["y"],
                kind="slinear",
                fill_value=1e6,
    ...
    def ci(self, coverage):
        ...
        if iscomplex:
            confint = confint.astype(complex)
        ...
        if self.type == "covariance":
            ...
            if iscomplex:
                confint[:, 0] = confint[:, 0] + 1j * np.maximum(
                    self.__lb, self.mean.imag - standardError
                )
                confint[:, 1] = confint[:, 1] + 1j * np.minimum(
                    self.__ub, self.mean.imag + standardError
                )
            ...
            confint[:, 0] = np.minimum(
                self.percentile((1 - p) * 100), np.amax(self.samples)
            )
    ...
    def propagate(self, model, lb=None, ub=None, samples=None):
        ...
        if np.size(modelfit) != np.size(lb) or np.size(modelfit) != np.size(ub):
            raise IndexError(
                "The 2nd and 3rd input arguments must have the same number of elements as the model output."
            )
        ...
        if self.type == "covariance":
            ...
            if iscomplex:
                ...
                modelcovmat = (
                    modelcovmat[np.ix_(Nreal, Nreal)]
                    + 1j * modelcovmat[np.ix_(Nimag, Nimag)]
                )
            ...
            for n in range(self.nparam):
                ...
                np.random.seed(0)
                sampled_parameters[n] = [
                    np.random.choice(values, p=pdf / sum(pdf)) for _ in range(Nsamples)
                ]
            # Convert to matrix
