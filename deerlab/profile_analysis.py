...
def profile_analysis(
    ...
    **kargs,
    ...
    threshold = (
        lambda coverage: noiselvl**2 * chi2.ppf(coverage, df=1) + fitresult.cost
    )
    ...
    for parameter in parameters:
        ...
        if grids is None:
            start = np.maximum(
                getattr(model, parameter).lb,
                getattr(fitresult, parameter)
                - 10 * getattr(fitresult, f"{parameter}Uncert").std,
            )
            stop = np.minimum(
                getattr(model, parameter).ub,
                getattr(fitresult, parameter)
                + 10 * getattr(fitresult, f"{parameter}Uncert").std,
            )
        ...
        uqresults[parameter] = UQResult(
            "profile",
            data=getattr(fitresult, parameter),
            profiles=profile,
            threshold=threshold,
            noiselvl=noiselvl,
        )
