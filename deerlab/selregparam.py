...
def selregparam(
    ...
    y, A, weights, _, __, noiselvl = dl.utils.parse_multidatasets(
        y, A, weights, noiselvl
    )
    ...
    evalalpha = lambda alpha: _evalalpha(
        alpha, y, A, L, solver, method, noiselvl, weights
    )
    ...
    if algorithm == "brent":
        ...
        functional, residuals, penalties, alphas_evaled = (
            np.array(0) for _ in range(4)
        )
        ...
        lga_opt = opt.fminbound(
            lambda lga: register_ouputs(evalalpha(10**lga)),
            lga_min,
            lga_max,
            xtol=0.01,
        )
        ...
        if candidates is None:
            alphaCandidates = 10 ** np.linspace(
                np.log10(searchrange[0]), np.log10(searchrange[1]), 60
            )
        else:
            alphaCandidates = np.atleast_1d(candidates)

        # Evaluate the full grid of alpha-candidates
        functional, residuals, penalties, alphas_evaled = tuple(
            zip(*[evalalpha(alpha) for alpha in alphaCandidates])
        )
        ...
        # L-curve maximum-curvature method (LC)
            ...
            functional = (d1Residual * d2Penalty - d2Residual * d1Penalty) / (
                d1Residual**2 + d1Penalty**2
            ) ** (3 / 2)
...
def _evalalpha(alpha, y, A, L, solver, selmethod, noiselvl, weights):
    ...
    # Robust Generalized Cross Validation (rGCV)
        ...
        f_ = (
            Residual**2
            / ((1 - np.trace(H) / N) ** 2)
            * (tuning + (1 - tuning) * np.trace(H**2) / N)
        )
    ...
    # Strong Robust Generalized Cross Validation (srGCV)
        ...
        f_ = (
            Residual**2
            / ((1 - np.trace(H) / N) ** 2)
            * (tuning + (1 - tuning) * np.trace(pA.T @ pA) / N)
        )
    ...
    # Normalized Cumulative Periodogram (NCP)
        ...
        for j in range(len(resPeriodogram) - 1):
            respowSpectrum[j] = np.linalg.norm(
                resPeriodogram[1 : j + 1], 1
            ) / np.linalg.norm(resPeriodogram[1:-1], 1)
