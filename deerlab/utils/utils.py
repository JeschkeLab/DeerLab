...
def parse_multidatasets(
    V_, K, weights, noiselvl, precondition=False, masks=None, subsets=None
):
    # ===============================================================================
    ...
    if type(V) is list and all([type(Vs) is np.ndarray for Vs in V]):
        ...
        raise TypeError(
            "The input signal(s) must be numpy array or a list of numpy arrays."
        )
    ...
    def prepareKernel(K, nSignals):
        ...
        if type(K) is list and all([type(Ks) is np.ndarray for Ks in K]):
            ...
            raise TypeError(
                "The input kernel(s) must be numpy array or a list of numpy arrays."
            )
        # Check that the same number of signals and kernel have been passed
        if nSignals != nKernels:
            raise KeyError(
                "The same number of kernels and signals must be specified as lists."
            )
        return K
    ...
    if (
        type(weights) is list
        or type(weights) is np.ndarray
        or not hasattr(weights, "__len__")
    ):
        weights = np.atleast_1d(weights)
        if len(weights) == 1:
            weights = np.repeat(weights, len(subset))
        if len(weights) != len(Vlist) and len(weights) != np.sum(
            [len(V) for V in Vlist]
        ):
            raise KeyError(
                "If multiple signals are passed, the same number of weights are required."
            )
        if len(weights) != np.sum([len(V) for V in Vlist]):
            weights_ = []
            for i in range(len(weights)):
                weights_ = np.concatenate(
                    (weights_, weights[i] * np.ones(len(Vlist[i])))
                )
            weights = weights_
    else:
        raise TypeError(
            "The input weights(s) must be numpy array or a list of numpy arrays."
        )
...
def der_snr(y):
    ...
    sigma = (
        1.482602
        / np.sqrt(6)
        * np.median(abs(2.0 * y[2 : n - 2] - y[0 : n - 4] - y[4:n]))
    )
...
def hccm(J, residual, mode="HC1"):
    ...
    if mode.upper() == "HC0":  # White,(1980),[1]
        ...
        # Estimate the data covariance matrix
        V = np.diag(residual**2.0 / ((1 - h) ** delta))
        ...
        # Estimate the data covariance matrix
        V = np.diag(residual**2.0 / (np.sqrt((1 - h) ** alpha)))
...
def gsvd(A, B):
    ...
    with warnings.catch_warnings():
        ...
        U = np.vstack(
            (
                np.zeros((q - m, 1), "double"),
                np.diag(C, max(0, q - m)).reshape(len(np.diag(C, max(0, q - m))), 1),
            )
        ) / np.vstack(
            (
                np.diag(S, 0).reshape(len(np.diag(S, 0)), 1),
                np.zeros((q - n, 1), "double"),
            )
        )
...
def multistarts(n, x0, lb, ub):
    ...
    if len(x0) != len(lb) or len(x0) != len(ub):
        raise ValueError(
            "The lower/upper bound size(s) are not compatible with the initial guess vector x0."
        )
