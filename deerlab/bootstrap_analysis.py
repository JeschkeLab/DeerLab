...
def bootstrap_analysis(
    ...
    for i in range(len(Vfit)):
        if len(Vexp[i]) != len(Vfit[i]):
            raise KeyError(
                f"The Vexp and Vfit #{i} must have the same number of elements."
            )
    if type(fcn) is not types.FunctionType:
        raise KeyError(
            "The 1st argument must be a callable function accepting dataset(s) as input."
        )
    ...
    def sample():
        ...
        for i in range(nSignals):
            ...
            if resampling == "gaussian":
                ...
                raise KeyError(
                    "Resampling method not found. Must be either 'gaussian' or 'residual'."
                )
        return Vsample
    ...
    for var in varargout:
        if not all(isnumeric(x) for x in var):
            raise ValueError(
                "Non-numeric output arguments by the analyzed function are not accepted."
            )

    # Check that the full bootstrap analysis will not exceed the memory limits
    memory_requirements = (
        np.sum([nSamples * np.atleast_1d(var).size * np.atleast_1d(var).itemsize]) / 1e9
    )  # GB
    if memory_requirements > memorylimit:
        raise MemoryError(
            f"The requested bootstrap analysis requires {memory_requirements:.2f}GB, exceeding the current memory limit {memorylimit:.2f}GB."
        )
    ...
    if verbose:
        print(f"Bootstrap analysis with {cores} cores:")
    out = _ProgressParallel(n_jobs=cores, total=nSamples, use_tqdm=verbose)(
        delayed(bootsample)() for _ in range(nSamples)
    )
    ...
    for varargout in out:
        ...
        for j in range(nout):
            ...
            if np.shape(out) != shapeout[j]:
                raise TypeError(
                    "Inconsistent output variable shape. One of the outputs of the analyzed function is changing its size/shape in between runs."
                )
