...
def deerload(fullbasename, plot=False, full_output=False, *args, **kwargs):
    ...
    * .DTA: data file
    ...
    if fileextension in [".DSC", ".DTA"]:
        ...
        raise ValueError(
            "Only Bruker BES3T files with extensions .DSC or .DTA are supported."
        )
        ...
        if Dimensions[index] == 1:
            ...
            if "IGD" == axistype:
                ...
                if "D" == parDESC[str(a + "FMT")]:
                    ...
                    raise ValueError(
                        f'Cannot read data format {a+"FMT"} for companion file {companionfilename}'
                    )
...
def read_description_file(DSCFileName):
    ...
    for nline, line in enumerate(allLines):
        ...
        if SectionName == "DSL" and not DeviceName:
            raise ValueError(
                "Found a line with key-value pair outside .DVC in #DSL layer."
            )
