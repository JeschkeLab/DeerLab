    ...
    def freeze(self, value):
        ...
        if np.any(value > self.ub) or np.any(value < self.lb):
            if len(np.atleast_1d(value)) > 1:
                ...
                raise ValueError(
                    f"Frozen value {value} is outside of the bounds {self.lb} and {self.ub}."
                )
...
class Model:
    ...
    def __init__(self, nonlinfcn, constants=None, signature=None):
        ...
        self.nonlinmodel = partial(
            self._model_with_constants, nonlinfcn, self._constantsInfo
        )
    ...
    def _parameter_list(self, order="alphabetical"):
        ...
        keylist = [
            param for param in dir(self) if isinstance(getattr(self, param), Parameter)
        ]
            ...
            for key in keylist:
                if isinstance(getattr(self, key).idx, np.ndarray):
                    ...
                    keylist = np.insert(
                        ...
                        n
                        * np.ones(
                            len(np.atleast_1d(getattr(self, key).idx)) - 1, dtype=int
                        ),
                        key,
                    )
    ...
    def _vecsort(self, paramlist):
        ...
        indices = np.concatenate(
            [
                np.atleast_1d(getattr(self, param).idx)
    ...
    def _split_linear(self, variable):
        ...
        linear = np.concatenate(
            [
                np.atleast_1d(getattr(self, param).linear)
                for param in dir(self)
                if isinstance(getattr(self, param), Parameter)
            ]
        )
    ...
    def _getvector(self, attribute):
        ...
        return np.concatenate(
            [
                np.atleast_1d(getattr(getattr(self, param), attribute))
                for param in dir(self)
                if isinstance(getattr(self, param), Parameter)
            ]
        )
    ...
    def addlinear(
        ...
        if vec > 1:
            ...
            newparam = Parameter(
                name=name,
                linear=np.full(vec, True),
                parent=self,
                idx=idx,
                par0=np.full(vec, par0),
                lb=np.full(vec, lb),
                ub=np.full(vec, ub),
                value=np.full(vec, None),
                frozen=np.full(vec, False),
                unit=unit,
                description=description,
            )
        else:
        ...
        if newparam.normalization is not None:
            if not callable(normalization):
                raise TypeError(
                    "The normalization function must be a callable function."
                )
    ...
    def __call__(self, *args, **kargs):
        ...
        if (len(args) + len(kargs)) != Nrequired:
            raise SyntaxError(
                f"The model requires {Nrequired} arguments, but {len(args)+len(kargs)} have been specified."
            )

        # Positional arguments
        args_constants = [
            np.atleast_1d(args[info["argidx"]])
            for info in self._constantsInfo
            if info["argidx"] < len(args)
        ]
        args_list = [
            np.atleast_1d(arg)
            for idx, arg in enumerate(args)
            if idx not in [info["argidx"] for info in self._constantsInfo]
        ]

        # Keywords arguments
        kargs_constants = [
            np.atleast_1d(kargs[info["argkey"]])
            for info in self._constantsInfo
            if info["argkey"] in kargs
        ]
        kargs_list = [
            np.atleast_1d(kargs[param])
            for param in self._parameter_list(order="vector")[len(args_list) :]
        ]
        ...
        if len(θ) != self.Nparam:
            raise SyntaxError(
                f"The model requires {self.Nparam} parameters, but {len(args_list)} were specified."
            )
    ...
    """
        ...
        for n, paramname in enumerate(self._parameter_list(order="vector")):
            ...
            linear_str = (
                "linear" if np.all(getattr(self, paramname).linear) else "nonlin"
            )
            ...
            table.append(
                [param_str, lb_str, ub_str, linear_str, frozen_str, unit_str, desc_str]
            )
...
class Penalty:
        ...
        def selectionfunctional(fitfcn, y, sigma, log10weight):
            ...
            if selection == "icc":
                ...
                if not np.all(covmat == 0):
                    icc = (
                        np.sum((y - yfit) ** 2 / sigma**2)
                        + np.sum(np.log(np.diag(covmat)))
                        + np.linalg.slogdet(covmat)[1]
                    )
                else:
    ...
    def optimize(self, fitfcn, y, sigma):
        ...
        if not self.weight.frozen:
            ...
            selectionFunctional = lambda log10weight: self.selectionfcn(
                fitfcn, y, sigma, log10weight
            )
...
def _outerOptimization(fitfcn, penalty_objects, y, sigma):
    ...
    # Otherwise, prepare to solve multiobjective problem
        ...
        thirdfcn = lambda y, *param: penalty_objects[2].optimize(
            lambda weight: fitfcn(y, [*param, weight]), y, sigma
        )[1]
        secondfcn = lambda y, *param: penalty_objects[1].optimize(
            lambda weight: fitfcn(y, [*param, weight, thirdfcn(y, *param, weight)]),
            y,
            sigma,
        )[1]
        fitfcn_ = lambda y: penalty_objects[0].optimize(
            lambda weight: fitfcn(
                y,
                [weight, secondfcn(y, weight), thirdfcn(y, weight, secondfcn(weight))],
            ),
            y,
            sigma,
        )[0]
        ...
        fitfcn_ = lambda y: penalty_objects[0].optimize(
            lambda weight: fitfcn(y, [weight, secondfcn(y, weight)]), y, sigma
        )[0]

    elif len(penalty_objects) == 1:
        fitfcn_ = lambda y: penalty_objects[0].optimize(
            lambda weight: fitfcn(y, [weight]), y, sigma
        )[0]
    else:
...
def _propagate(
    fitresults, params, _paramlist, paramidx, model, *constants, lb=None, ub=None
):
    # ----------------------------------------------------------------------------
    """
    ...
    subset = [
        paramidx[np.where(np.asarray(_paramlist) == param)[0][0]]
        for param in modelparam
    ]
    # Propagate the uncertainty from that subset to the model
    modeluq = fitresults.paramUncert.propagate(
        lambda param: model(*constants, *[param[s] for s in subset]), lb, ub
    )
    return modeluq
...
def _print_fitresults(fitresult, model):
    ...
    for n in range(Ndatasets):
        ...
        rmsd = stats[n]["rmsd"]
        aic = stats[n]["aic"]
        noiselvl, chi2red, rmsd, aic = [
            f"{var:.3f}" if var < 1e4 else f"{var:.3g}"
            for var in [noiselvl, chi2red, rmsd, aic]
        ]
    ...
    table = []
    table.append(
        [f"Parameter", "Value", "95%-Confidence interval", "Unit", "Description"]
    )  # Header
    alignment = ["<", "<", "<", "^", "<"]  # Alignment
            ...
            if np.any(getattr(model, param).frozen):
                ...
                if getattr(fitresult, param + "Uncert").type == "void":
                    ...
                    value, ci_lower, ci_upper = [
                        f"{var:.3f}" if var < 1e4 else f"{var:.3g}"
                        for var in [value, ci_lower, ci_upper]
                    ]
...
def insert_snlls_optionals_docstrings():
    ...
    def decorator(func):
        func.__doc__ = func.__doc__.replace(
            "snlls_keyargs_docstrings", snlls_keyargs_docs
        )
        return func
...
def fit(
    ...
    **kwargs,
    ...
    if len(constants) != required:
        raise SyntaxError(
            f"The input model requires {required} constant(s) to be specified. Specify them via fit(model,y,*constants)."
        )
        ...
        if np.all(param.linear):
            if param.normalization is not None:
                model.addnonlinear(
                    ...
                    ub=np.inf,
                    par0=1,
                    description=f"Normalization factor of {key}",
                )
    ...
    if np.any(par0 == None):
        raise RuntimeError(
            f"It appears some start values (par0) have not been specified. Either specify them in the model definition or using the keyword."
        )
    ...
    if type(y) is not list:
        y = [y]
    ysplit = y.copy()
    y, _, weights, mask, ysubsets, noiselvl = parse_multidatasets(
        y, None, weights, noiselvl, precondition=False, masks=mask
    )
    sigmas = np.concatenate(
        [np.full_like(yset, sigma) for sigma, yset in zip(noiselvl, ysplit)]
    )
    ...
    if penalties is not None:
        ...
        for penalty in penalties:
            ...
            subsets = [
                getattr(
                    model, modelparam[np.where(np.asarray(modelparam) == param)[0][0]]
                ).idx
                for param in penalty.signature
            ]
            # Adapt the signature of penaltyfcn for snlls
            penaltyfcns.append(
                lambda pnonlin, plin, weight: penalty.penaltyfcn(
                    weight,
                    *[np.concatenate([pnonlin, plin])[subset] for subset in subsets],
                )
            )

        # Prepare the penalties to input to snlls
        extrapenalties = lambda weights: [
            lambda nonlin, lin: penaltyfcn(nonlin, lin, weight)
            for penaltyfcn, weight in zip(penaltyfcns, weights)
        ]
    else:
    ...
    fitfcn = lambda y, penweights: snlls(
        y,
        Amodel_fcn,
        par0,
        lb=lb,
        ...
        weights=weights,
        subsets=ysubsets,
        lin_frozen=linfrozen,
        nonlin_frozen=nonlinfrozen,
        regparam=regparam,
        reg=reg,
        regparamrange=regparamrange,
        noiselvl=noiselvl,
        extrapenalty=extrapenalties(penweights),
        **kwargs,
    )
    ...
    if bootstrap > 0:
        ...
        def bootstrap_fcn(ysim):
            ...
            if not isinstance(fit.model, list):
                fit.model = [fit.model]
            return (fit.param, *fit.model)

        # Bootstrapped uncertainty quantification
        param_uq = bootstrap_analysis(
            bootstrap_fcn, ysplit, fitresults.model, samples=bootstrap
        )
        ...
        fitresults.paramUncert = UQResult(
            "bootstrap", data=param_uq[0].samples, lb=paramlb, ub=paramub
        )
    ...
    FitResult_param = {
        key: fitvalue if len(fitvalue) > 1 else fitvalue[0]
        for key, fitvalue in zip(keys, [fitresults.param[idx] for idx in param_idx])
    }
    # Dictionary of parameter names and fit uncertainties
    FitResult_paramuq = {
        f"{key}Uncert": model._getparamuq(fitresults.paramUncert, idx)
        for key, idx in zip(keys, param_idx)
    }
    # Dictionary of other fit quantities of interest
    FitResult_dict = {
        key: getattr(fitresults, key)
        for key in [
            "param",
            "paramUncert",
            "model",
    ...
    FitResult_param_, FitResult_paramuq_ = (
        FitResult_param.copy(),
        FitResult_paramuq.copy(),
    )
    if normalization:
        for key in keys:
            param = getattr(model, key)
            ...
            if np.all(param.linear):
                if param.normalization is not None:
                    ...
                    FitResult_param_[key] = param.normalization(
                        FitResult_param_[key]
                    )  # Normalized value
                    FitResult_param_[f"{key}_scale"] = np.mean(
                        non_normalized / FitResult_param_[key]
                    )  # Normalization factor
                    FitResult_paramuq_[f"{key}Uncert"] = FitResult_paramuq_[
                        f"{key}Uncert"
                    ].propagate(
                        lambda x: x / FitResult_param_[f"{key}_scale"],
                        lb=param.lb,
                        ub=param.ub,
                    )  # Normalization of the uncertainty
...
# ---------------------------------------------------------------------
def _linked_model_with_constants(
    nonlinfcn, mapping, constantsInfo, linear_reduce_idx, *inputargs
):
    ...
    Amapped = np.vstack(
        [np.sum(np.atleast_2d(A[:, idx]), axis=1) for idx in linear_reduce_idx]
    ).T
    return Amapped
...
def link(model, **links):
    ...
    def _linkparameter(model, parameters, newname):
        ...
        linear_reduce_idx = [
            np.where(mapping_linear == n)[0].tolist() for n in np.unique(mapping_linear)
        ]

        # Redefine the non-linear part of the model function
        model.nonlinmodel = partial(
            _linked_model_with_constants,
            nonlinfcn,
            mapping,
            model._constantsInfo,
            linear_reduce_idx,
        )
    ...
    for key in links.keys():
        newmodel.signature = [
            key if arg == links[key][0] else arg for arg in newmodel.signature
        ]
...
def _combined_nonlinmodel(
    ...
    *inputargs,
    ...
    for n, nonlinfcn in enumerate(nonlinfcns):
        ...
        Amatrix = np.atleast_2d(
            nonlinfcn(*constants[const_subsets[n], :], *param[subsets_nonlin[n]])
        )
...
# ---------------------------------------------------------------------
def _split_output(
    nonlinfcns, Nlins, Nconst, const_subsets, subsets_nonlin, y, *inputargs
):
...
def _combinemodels(mode, *inputmodels, addweights=False):
    ...
    if addweights:
        ...
        for n, (model, nonlinfcn) in enumerate(
            zip(models, [model.nonlinmodel for model in models])
        ):
            ...
            weightedModel = Model(
                make_weighted_comb(nonlinfcn), constants=constants, signature=signature
            )
            for name in model._parameter_list(order="vector"):
                if np.any(getattr(model, name).linear):
                    weightedModel.addlinear(
                        name, vec=len(np.atleast_1d(getattr(model, name).idx))
                    )
                getattr(weightedModel, name).set(
                    **_importparameter(getattr(model, name))
                )
            getattr(weightedModel, "weight").set(
                lb=0, par0=1, description="Weighting factor"
            )
    ...
    for n, model in enumerate(models):
        ...
        arelinear = np.concatenate(
            [arelinear, model._vecsort(model._getvector("linear"))]
        )
        lin_normalizations += [
            getattr(model, param).normalization
            for param in model._parameter_list()
            if hasattr(getattr(model, param), "normalization")
        ]
        ...
        for oldkey, newkey in zip(oldargs, newarguments):
            if isinstance(getattr(model, oldkey).idx, np.ndarray):
                newarguments = np.insert(
                    newarguments,
                    i
                    * np.ones(
                        len(np.atleast_1d(getattr(model, oldkey).idx)) - 1, dtype=int
                    ),
                    newkey,
                ).tolist()
    ...
    if mode == "merge":
        ...
        setattr(
            ...
            "_posteval_fcn",
            partial(
                _split_output, nonlinfcns, Nlins, Nconst, const_subsets, subsets_nonlin
            ),
        )

    # Add the linear parameters from the subset models
    lin_param_set = []
    for param, lin_normalization in zip(
        _unique_ordered(lin_params), lin_normalizations
    ):
        lin_param_set.append(
            {
                "name": param,
                "vec": np.sum(lin_params == param),
                "normalization": lin_normalization,
            }
        )

    for lparam in lin_param_set:
        combinedModel.addlinear(
            lparam["name"], vec=lparam["vec"], normalization=lparam["normalization"]
        )
...
# ---------------------------------------------------------------------
def _dependency_model_with_constants(
    function, nonlinfcn, constantsInfo, arguments_idx, dependent_idx, *inputargs
):
    # Redistribute the input parameter vector according to the mapping vector
    Nconstants = len(constantsInfo)
    constants = inputargs[:Nconstants]
...
def relate(model, **functions):
    ...
    def _relate(model, function, dependent_name):
        ...
        if dependent_name not in model_parameters:
            raise KeyError(
                f"The assigned parameter '{dependent_name}' is not a parameter of the input model."
            )
        ...
        for arg in arguments_names:
            if arg not in model_parameters:
                raise KeyError(
                    f"The function argument '{arg}' is not a parameter of the input model."
                )
        ...
        arguments_idx = np.concatenate(
            [np.where(nonlinparams == name)[0] for name in arguments_names]
        )
        dependent_idx = dependent_idx[0]

        # Redefine the non-linear part of the model function
        model.nonlinmodel = partial(
            _dependency_model_with_constants,
            function,
            nonlinfcn,
            model._constantsInfo,
            arguments_idx,
    ...
    arguments = [
        inspect.getfullargspec(functions[dependent]).args for dependent in dependents
    ]
    ...
    for _ in range(maxtrials):
        ...
        raise RuntimeError(
            "There are cyclic relationships in the parameter definitions that could not be resolved."
        )
