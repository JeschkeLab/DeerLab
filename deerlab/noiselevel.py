...
def noiselevel(V, mode="der", *args):
    ...
    * ``sigma = noiselevel(V,'complex')``: If the input signal ``V`` contains an imaginary component, the noise
    ...
    if mode == "scans":
        ...
        if V.ndim != 2:
            raise KeyError(
                "For the 'scans' method, the input signal must be a 2D-matrix."
            )
        # Estimate standard deviations for all time point, and average over scans
        if shape(V)[1] < 10:
            raise Warning(
                "Only a few scans are given. Noise standard deviation estimate will be inaccurate."
            )
        sigma = std(V, 1)
        sigma = mean(sigma)
        ...
        if args:
            if len(args) != 1:
                raise KeyError(
                    "For the 'movmean' method, only the windows size can be specified."
                )
            winsize = args
        else:
            raise KeyError(
                "For the 'movmean' method, the windows size must be specified."
            )
        ...
        if args:
            if len(args) != 2:
                raise KeyError(
                    "For the 'savgol' method, only the size and order can be specified."
                )
            winsize, order = args
        else:
            raise KeyError(
                "For the 'savgol' method, the size and order must be specified."
            )
        ...
        if all(isreal(V)):
            raise TypeError(
                "For the 'complex' method, the input signal must be complex-valued."
            )
        ...
        if args:
            ...
            raise KeyError(
                "For the 'reference' method, a reference signal must be provided."
            )
        if len(V) != len(Vref):
            raise TypeError(
                "The input and reference signal must have the same number of elements."
            )
        sigma = std(V - Vref)
