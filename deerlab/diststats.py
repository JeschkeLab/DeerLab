...
def diststats(r, P, Puq=None, verbose=False, threshold=None):
    ...
    if not np.all(np.diff(r) > 0):
        raise KeyError(
            "The distance axis (1st argument) must be a monotnously increasing vector."
        )
    ...
    def iqmfcn(P):
        ...
        return E(
            r[IQrange],
            P[IQrange] / np.trapz(normalize(P)[IQrange], r[IQrange]),
            r[IQrange],
        )
    ...
    entropyfcn = lambda P: -E(
        np.log(np.maximum(np.finfo(float).eps, normalize(P))), P, r
    )
...
def _print_estimators(r, estimators, uq):
    ...
    if uq is None:
        ...
        print(f"Range                    {min(r):.2f}-{max(r):.2f} nm")
        print(
            f'Mean                     {estimators["mean"]:.2f} ({uq["mean"].ci(95)[0]:.2f},{uq["mean"].ci(95)[1]:.2f}) nm'
        )
        print(
            f'Median                   {estimators["median"]:.2f} ({uq["median"].ci(95)[0]:.2f},{uq["median"].ci(95)[1]:.2f}) nm'
        )
        print(
            f'Interquartile mean       {estimators["iqm"]:.2f} ({uq["iqm"].ci(95)[0]:.2f},{uq["iqm"].ci(95)[1]:.2f}) nm'
        )
        print(f'Mode                     {estimators["mode"]:.2f} nm')
        print("-------------------------------------------------")
        print("Spread")
        print("-------------------------------------------------")
        print(
            f'Standard deviation       {estimators["std"]:.2f} ({uq["std"].ci(95)[0]:.2f},{uq["std"].ci(95)[1]:.2f}) nm'
        )
        print(
            f'Mean absolute deviation  {estimators["mad"]:.2f} ({uq["mad"].ci(95)[0]:.2f},{uq["mad"].ci(95)[1]:.2f}) nm'
        )
        print(
            f'Interquartile range      {estimators["iqr"]:.2f} ({uq["iqr"].ci(95)[0]:.2f},{uq["iqr"].ci(95)[1]:.2f}) nm'
        )
        print(
            f'Variance                 {estimators["var"]:.2f} ({uq["var"].ci(95)[0]:.2f},{uq["var"].ci(95)[1]:.2f}) nm²'
        )
        print("-------------------------------------------------")
        print("Shape")
        print("-------------------------------------------------")
        print(f'Modality                 {estimators["modality"]}')
        print(
            f'Skewness                 {estimators["skewness"]:.2f} ({uq["skewness"].ci(95)[0]:.2f},{uq["skewness"].ci(95)[1]:.2f}) '
        )
        print(
            f'Kurtosis                 {estimators["kurtosis"]:.2f} ({uq["kurtosis"].ci(95)[0]:.2f},{uq["kurtosis"].ci(95)[1]:.2f}) '
        )
        print("-------------------------------------------------")
