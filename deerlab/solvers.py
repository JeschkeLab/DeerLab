...
def _check_bounds(lb, ub, par0=None, N=None):
    ...
    if par0 is not None:
        if len(lb) != len(par0) or len(ub) != len(par0):
            raise TypeError(
                "The lower/upper bounds and start values must have the same number of elements"
            )
    ...
    if N is not None:
        if len(lb) != N or len(ub) != N:
            raise TypeError(
                f"The lower/upper bounds and start values must have {N} elements"
            )
    if np.any(ub < lb):
...
def _optimize_scale(y, yfit, subsets):
    ...
    for subset in subsets:
        yfit_, y_ = (
            np.atleast_2d(y[subset]) for y in [yfit, y]
        )  # Rescale the subsets corresponding to each signal
...
def _unfrozen_subset(param, frozen, parfrozen):
    ...
    param = np.atleast_1d(
        [parfrozen[n] if frozen[n] else param.pop(0) for n in range(len(frozen))]
    )
    return param
...
def snlls(
    ...
    y, Amodel, weights, mask, subsets, noiselvl = parse_multidatasets(
        y, Amodel, weights, noiselvl, masks=mask, subsets=subsets
    )
    ...
    if np.shape(A0)[0] != np.shape(y)[0]:
        raise RuntimeError(
            f"The number of elements ({np.shape(A0)[0]}) in the model's output does not match the number of elements ({np.shape(y)[0]}) in the data."
        )
    ...
    if includeExtrapenalty:
        ...
        for penalty in extrapenalty:
            if not callable(penalty):
                raise TypeError(
                    "The keyword argument 'extrapenalty' must be a callable function or a list thereof."
                )
    ...
    _Amodel = Amodel
    Amodel = lambda param: _Amodel(
        _unfrozen_subset(param, nonlin_frozen, nonlin_parfrozen)
    )

    if includeExtrapenalty:
        extrapenalty_ = [penalty for penalty in extrapenalty]
        extrapenalty = [
            lambda pnonlin, plin: penalty(
                _unfrozen_subset(pnonlin, nonlin_frozen, nonlin_parfrozen), plin
            )
            for penalty in extrapenalty_
        ]
    ...
    if Nlin_notfrozen > 0:
        (
            ...
            A0red, lbl_red, ubl_red, reg, regop, lin_tol, lin_maxiter, nnlsSolver
        )
    else:
    ...
    def linear_problem(y, A, optimize_alpha, alpha):
        ...
        AtA, Aty = _lsqcomponents(
            (y - yfrozen)[mask], Ared[mask, :], L, alpha, weights=weights[mask]
        )
        ...
        if includeRegularization:
            if type(regparam) is str:
                if Nnonlin_notfrozen > 0:
                    ...
                    if check and all(
                        abs(par_prev - p) / (p + np.finfo(np.float64).eps) < alphareopt
                    ):
    ...
    # -------------------------------------------------------------------
        ...
        if multistart > 1 and not nonLinearBounded:
            raise TypeError(
                "Multistart optimization cannot be used with unconstrained non-linear parameters."
            )
    ...
    _ResidualsFcn = lambda nonlinfit: ResidualsFcn(
        _unfrozen_subset_inv(nonlinfit, nonlin_frozen)
    )
            ...
            for penalty in extrapenalty:
                Jlin = np.concatenate(
                    (
                        Jlin,
                        Jacobian(
                            lambda plin: penalty(nonlinfit, plin), linfit, lbl, ubl
                        ),
                    )
                )
        if includeRegularization:
            Jlin = np.concatenate(
                (Jlin, _penalty_augmentation(alpha, L, linfit, "Jacobian"))
            )
        Jlin *= scale
        # Full Jacobian
    ...
    if complexy:
        ...
        def ymodel(n):
            return (
                lambda p: ymodel_(n)(p)
                + 1j * (Amodel(p[nonlin_idx]) @ p[lin_idx])[imagsubsets[n]]
            )
    ...
    return FitResult(
        ...
        param=parfit,
        model=modelfit,
        nonlinUncert=paramuq_nonlin,
        linUncert=paramuq_lin,
        paramUncert=paramuq,
        modelUncert=modelfituq,
        regparam=alpha,
        plot=plotfcn,
        stats=stats,
        cost=fvals,
...
def fnnls(AtA, Atb, tol=None, maxiter=None, verbose=False):
    ...
    while np.any(w > tol) and np.any(~passive):
        ...
        while any((x_ <= tol) & passive) and iIteration < maxIterations:
            ...
            if np.sum(passive) == 1:
                ...
                x_[passive] = np.linalg.solve(
                    AtA[np.ix_(passive, passive)], Atb[passive]
                )
    ...
    if verbose:
        if unsolvable:
            print(
                "Optimization stopped because the solution cannot be further changed. \n"
            )
        elif any(~passive):
            print(
                "Optimization stopped because the active set has been completely emptied. \n"
            )
        elif w > tol:
            print(
                f"Optimization stopped because the gradient (w) is inferior than the tolerance value TolFun = {tol:.6e}. \n"
            )
        else:
