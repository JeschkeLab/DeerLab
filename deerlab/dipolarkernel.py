...
def ω0(g):
    return (
        (μ0 / 2) * μB**2 * g[0] * g[1] / h * 1e21
    )  # Hz m^3 -> MHz nm^3 -> rad μs^-1 nm^3
...
def dipolarkernel(
    ...
    *,
    ...
    if method == "integral":
        if complex:
            raise KeyError(
                "Complex-valued kernels cannot be computed by the 'integral' method."
            )
    ...
    paths = [
        np.concatenate([np.atleast_1d(p) for p in path]).astype(float) for path in paths
    ]
    ...
    K0 = lambda t: elementarykernel(
        t, r, method, excbandwidth, nKnots, g, orisel, complex
    )
...
def _Cgrid(ωr, t, ωex, q, complex):
    ...
    if complex:
        C = np.exp(
            -1j
            * ωr[np.newaxis, :, np.newaxis]
            * q[np.newaxis, np.newaxis, :]
        ...
        C = np.cos(
            ωr[np.newaxis, :, np.newaxis]
            * q[np.newaxis, np.newaxis, :]
            * abs(t[:, np.newaxis, np.newaxis])
        )
    # If given, include limited excitation bandwidth
    if not np.isinf(ωex):
        C = C * np.exp(
            -((ωr[np.newaxis, :, np.newaxis] * q[np.newaxis, np.newaxis, :]) ** 2)
            / ωex**2
        )
    return C
...
def elementarykernel(t, r, method, ωex, nKnots, g, Pθ, complex):
    ...
    if orientationselection:
        ...
        Pθnorm, _ = scipy.integrate.quad(
            lambda cosθ: Pθ_(np.arccos(cosθ)), 0, 1, limit=1000
        )
