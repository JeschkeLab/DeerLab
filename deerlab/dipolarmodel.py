...
def dipolarmodel(
    ...
    if npathways == 1:
        ...
        getattr(PathsModel, f"mod").set(
            lb=0, ub=1, par0=0.01, description=f"Modulation depth", unit=""
        )
        getattr(PathsModel, f"reftime").set(
            par0=0, description=f"Refocusing time", unit="μs"
        )
    else:
            ...
            for param in model._parameter_list(order="vector"):
                if np.any(getattr(model, param).linear):
                    ...
                    linearparam.append(
                        {
                            "name": param,
                            "vec": len(np.atleast_1d(getattr(model, param).idx)),
                            "normalization": getattr(model, param).normalization,
                        }
                    )
    ...
    if Bmodel is None:
        ...
        Bsubset = np.zeros(
            Bmodel.Nnonlin - ("lam" in Bmodel._parameter_list()), dtype=int
        )

    # Determine subset indices based on main function signature
    idx = 0
    for model, subset in zip(
        [Pmodel, Bmodel, PathsModel], [Psubset, Bsubset, PathsSubset]
    ):
        if model is not None:
...
def dipolarpenalty(Pmodel, r, type, selection=None):
    ...
    # If include smoothness penalty
        ...
        raise KeyError(
            f"The requested {type} is not a valid penalty. Must be 'compactness' or 'smoothness'."
        )
...
def _checkpathways(pathways, Nmax):
    ...
    if np.any(np.array(pathways) < 1) or not np.all([not p % 1 for p in pathways]):
        raise ValueError(
            f"The pathways must be specified by integer numbers in the range 1-{Nmax}."
        )
